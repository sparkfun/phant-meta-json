...
function PhantStorage(config) {
  ...
  if (! fs.existsSync(storage.directory)) {
    fs.mkdirSync(storage.directory);
  }

  // make sure streams directory exists
  if (! fs.existsSync(path.join(storage.directory, 'streams'))) {
    fs.mkdirSync(path.join(storage.directory, 'streams'));
  }
...
app.directory = path.join(__dirname, '..', '.tmp');
...
app.getRecords = function(id, callback) {
  ...
  var file = path.join(this.directory, 'streams', id + '.json');
...
app.addRecord = function(id, data, callback) {
  ...
  this.getStreams(function(streams) {
    ...
    self.getRecords(id, function(err, records) {

      var file = path.join(self.directory, 'streams', id + '.json');
      ...
      fs.writeFile(file, records, function(err) {
